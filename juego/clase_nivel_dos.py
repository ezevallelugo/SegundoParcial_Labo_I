...
class NivelDos (Nivel):
    def __init__(self, pantalla: pygame.Surface) -> None:
        ...
        #Fondo
        fondo = pygame.image.load("RECURSOS/Entorno/biblioteca.jpg")
        fondo_escalado = pygame.transform.scale(fondo, (W,H))
        x = 0
        pantalla.blit(fondo_escalado, (x, 0))
        #Sonido
        pygame.mixer.init()
        pygame.mixer.music.load("RECURSOS/Sonidos/biblioteca.mp3")
        ...
        plataforma_uno = Plataforma(plataforma_superficie, (300,700), None)
        plataforma_dos = Plataforma(plataforma_superficie, (10,500), None)
        plataforma_tres = Plataforma(plataforma_superficie, (300,300), None)
        plataforma_cuatro = Plataforma(plataforma_superficie, (700,300), None)
        plataforma_cinco = Plataforma(plataforma_superficie, (1000,500), None)
        plataforma_seis = Plataforma(plataforma_superficie, (700,700), None)
        plataforma_siete = Plataforma(plataforma_superficie, (1300,700), None)

        columna_superficie = pygame.image.load("RECURSOS/Entorno/columna1.png")
        columna_superficie = pygame.transform.scale(columna_superficie, (100,600))
        columna_uno = Plataforma(columna_superficie,(plataforma_uno.get_rectangulo().right,piso.get_rectangulo().top - 600), None)
        columna_dos = Plataforma(columna_superficie,(plataforma_cinco.get_rectangulo().right,plataforma_cinco.get_rectangulo().bottom - 600), None)
        ...
        llave_uno = Item(item_calavera["idle"][0],(plataforma_siete.get_rectangulo().centerx, plataforma_siete.get_rectangulo().top - 70), item_calavera)
        ...
        trampa_dos = Item(dict_trampa["idle"][0],(plataforma_cuatro.get_rectangulo().centerx - 30, plataforma_cuatro.get_rectangulo().top - 50), dict_trampa)
        ...
        pocion_uno = Recompensa(dict_pocion_salud["idle"][0],(columna_uno.get_rectangulo().centerx,columna_uno.get_rectangulo().top - 60),dict_pocion_salud)
        pocion_dos = Recompensa(dict_pocion_salud["idle"][0],(plataforma_seis.get_rectangulo().centerx,plataforma_seis.get_rectangulo().top - 60),dict_pocion_salud)
        ...
        with sqlite3.connect("juego_pygame.db") as conexion:
            try: 
                sentencia = '''
                            SELECT Nombre,Nivel_uno,Puntaje_actual,Permiso FROM Jugadores WHERE Id = 1
                            '''
        ...
        super().__init__(pantalla, personaje, lista_plataformas, lista_plataformas_lados, lista_enemigos, lista_recompensas, lista_trampas, lista_pociones, lista_llaves, tiempo_final, datos_jugador, 2, fondo_escalado)
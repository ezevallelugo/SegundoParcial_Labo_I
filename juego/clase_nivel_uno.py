...
class NivelUno (Nivel):
    def __init__(self, pantalla: pygame.Surface) -> None:
        ...
        #Fondo
        fondo = pygame.image.load("RECURSOS/Entorno/ciudad2.jpg")
        fondo_escalado = pygame.transform.scale(fondo, (W,H))
        x = 0
        pantalla.blit(fondo_escalado, (x, 0))
        #Sonido
        pygame.mixer.init()
        pygame.mixer.music.load("RECURSOS/Sonidos/ciudad_subterranea.mp3")
        ...
        plataforma_dos = Plataforma(plataforma_superficie, (800,600), None)
        plataforma_tres = Plataforma(plataforma_superficie, (1300,500), None)
        plataforma_cuatro = Plataforma(plataforma_superficie, (800,300), None)
        plataforma_cinco = Plataforma(plataforma_superficie, (300,400), None)

        columna_superficie = pygame.image.load("RECURSOS/Entorno/columna1.png")
        columna_superficie = pygame.transform.scale(columna_superficie, (100,300))
        columna_uno = Plataforma(columna_superficie,(piso.get_rectangulo().left,piso.get_rectangulo().top - 300), None)
        columna_dos = Plataforma(columna_superficie,(plataforma_cinco.get_rectangulo().left-100,plataforma_cinco.get_rectangulo().bottom - 300), None)
        ...
        pocion_dos = Recompensa(dict_pocion_salud["idle"][0],(plataforma_cuatro.get_rectangulo().centerx,plataforma_cuatro.get_rectangulo().top - 60),dict_pocion_salud)
        ...
        with sqlite3.connect("juego_pygame.db") as conexion:
            try: 
                ...
                datos_jugador["Puntaje_actual"] = 0
                datos_jugador["Puntaje_anterior"] = 0
                print("Datos extraidos con exito") 
            except Exception as e:
                print(f"Error. No se pudo extraer los datos.\nRazon: {e}")

        super().__init__(pantalla, personaje, lista_plataformas, lista_plataformas_lados, lista_enemigos, lista_recompensas, lista_trampas, lista_pociones, lista_llaves, tiempo_final, datos_jugador, 1, fondo_escalado)